...
function processInstruction(instruction, registers) {
	...
	const registerName = instructionList[0];
	const operation = instructionList[1];
	const value = instructionList[2];
	// index 3 is always "if"
	const conditionalRegister = instructionList[4];
	const condition = instructionList[5];
	const conditionalValue = instructionList[6];

	setRegisterIfUndefined(registers, registerName);
	setRegisterIfUndefined(registers, conditionalRegister);

	if(evaluateCondition(registers, conditionalRegister, condition, conditionalValue)) {
		applyValue(registers, registerName, operation, value);
	}
}

function setRegisterIfUndefined(registers, regName) {
	if(!registers.hasOwnProperty(regName)) {
		registers[regName] = 0;
	}
}

function evaluateCondition(registers, conditionalRegister, condition, conditionalValue) {
	return eval(`registers.${conditionalRegister} ${condition} ${conditionalValue}`);
}

function applyValue(registers, registerName, operation, value) {
	const operationFunction = operation === 'inc' ? (registers, registerName, value) => registers[registerName] += parseInt(value) : (registers, registerName, value) => registers[registerName] -= parseInt(value);
	operationFunction(registers, registerName, value);
}

function getLargetRegisterValue(registers) {
	return Object.keys(registers).map(key => registers[key]).reduce((result, current) => current > result ? current : result, Number.MIN_SAFE_INTEGER);
}

const registers = {};
input.forEach(inst => processInstruction(inst, registers));

console.log(registers);
console.log(getLargetRegisterValue(registers));
